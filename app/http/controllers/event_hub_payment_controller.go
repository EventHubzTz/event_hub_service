...
func (c eventHubPaymentController) ContributionPushUSSD(ctx *fiber.Ctx) error {
	type PaymentTransactionData struct {
		Results       string `json:"results"`
		Message       string `json:"message"`
		TransactionID string `json:"transaction_id"`
	...
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST OF PUSH
	     USSD
	---------------------------------------------------------*/
	var request payments.EventHubContributionPaymentRequest
	/*---------------------------------------------------------
	 02. PARSING THE BODY OF THE INCOMING REQUEST
	----------------------------------------------------------*/
	err := ctx.BodyParser(&request)
	...
	/*--------------------------------------------------------------------
	 16. ADD PAYMENT TRANSACTION
	-----------------------------------------------------------------------*/
	err = service.EventHubPaymentService.AddContributionTransaction(request.ToModel())
	if err != nil {
		errorPaymentData.Message = err.Error()
		return response.DataListErrorResponse(errorPaymentData, fiber.StatusBadRequest, ctx)
	}
...
func (c eventHubPaymentController) GetContributionTransactions(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST OF GETTING
	     CONTENTS
	---------------------------------------------------------*/
	...
	-------------------------------------------------------------------*/
	user := service.EventHubUserTokenService.GetUserFromLocal(ctx)
	paymentTransactions, err := service.EventHubPaymentService.GetContributionTransactions(pagination, user.Role, request.Query, request.Status, request.PhoneNumber, user.Id)
	/*---------------------------------------------------------
	 05. CHECK IF ERROR IS AVAILABLE AND RETURN ERROR RESPONSE
	----------------------------------------------------------*/
	if err != nil {
...
func (c eventHubPaymentController) GetContributionByTransactionID(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST OF GETTING
	     CONTENTS
	---------------------------------------------------------*/
	...
	/*---------------------------------------------------------
	 04. GET TRANSACTION
	----------------------------------------------------------*/
	transaction := repositories.EventHubPaymentRepository.GetContributionByTransactionID(request.TransactionID)
	if transaction == nil {
		return response.ErrorResponse("Transaction does not exist in the system", fiber.StatusInternalServerError, ctx)
	}
...
func (c eventHubPaymentController) UpdatePaymentStatus(ctx *fiber.Ctx) error {
	...
	/*---------------------------------------------------------
	 11. GET CONTRIBUTION TRANSACTION
	----------------------------------------------------------*/
	contributionTransaction := repositories.EventHubPaymentRepository.GetContributionByTransactionID(request.Reference)
	if contributionTransaction != nil {
		/*---------------------------------------------------------
		 12. UPDATE PAYMENT STATUS
		----------------------------------------------------------*/
		dbResponse := repositories.EventHubPaymentRepository.UpdateContributionPaymentStatus(request.Reference, constants.Completed)
		if dbResponse.RowsAffected == 0 {
			return response.ErrorResponse("Failed to update payment status!", fiber.StatusBadRequest, ctx)
		}
	}
	/*-----------------------------------------------------------------
	 13. UPDATE TANZANIA GOSPEL MUSIC AWARDS PAYMENTS
	-------------------------------------------------------------------*/
...
func (c eventHubPaymentController) AddPaymentRequest(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST OF GETTING
	     CONTENTS
	---------------------------------------------------------*/
	var request payments.EventHubRequestPaymentRequest
	/*---------------------------------------------------------
	 02. PARSING THE BODY OF THE INCOMING REQUEST
	----------------------------------------------------------*/
	err := ctx.BodyParser(&request)
	...
	/*--------------------------------------------------------------------
	 04. ADD PAYMENT REQUEST
	-----------------------------------------------------------------------*/
	err = service.EventHubPaymentService.AddPaymentRequest(request.ToModel())
	if err != nil {
		return response.ErrorResponse(err.Error(), fiber.StatusInternalServerError, ctx)
	}

	return response.SuccessResponse("Payment request added successful on "+date_utils.GetNowString(), fiber.StatusOK, ctx)
}

func (c eventHubPaymentController) GetPaymentRequestsByPagination(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	...
	/*-----------------------------------------------------------------
	 04. GET PAYMENT REQUESTS AND GET ERROR IF IS AVAILABLE
	-------------------------------------------------------------------*/
	paymentTransactions, err := service.EventHubPaymentService.GetPaymentRequestsByPagination(pagination, request.Query)
	/*---------------------------------------------------------
	 05. CHECK IF ERROR IS AVAILABLE AND RETURN ERROR RESPONSE
	----------------------------------------------------------*/
	if err != nil {
...
func (c eventHubPaymentController) AddOtherPayment(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST OF GETTING
	     CONTENTS
	---------------------------------------------------------*/
	var request payments.EventHubOtherPaymentRequest
	/*---------------------------------------------------------
	 02. PARSING THE BODY OF THE INCOMING REQUEST
	----------------------------------------------------------*/
	err := ctx.BodyParser(&request)
	...
	/*--------------------------------------------------------------------
	 04. ADD OTHER PAYMENT
	-----------------------------------------------------------------------*/
	err = service.EventHubPaymentService.AddOtherPayment(request.ToModel())
	if err != nil {
		return response.ErrorResponse(err.Error(), fiber.StatusInternalServerError, ctx)
	}
...
func (c eventHubPaymentController) GetOtherPaymentsByPagination(ctx *fiber.Ctx) error {
	/*-------------------------------------------------------
	 01. INITIATING VARIABLE FOR THE REQUEST
	---------------------------------------------------------*/
	var request events.EventHubEventsGetsRequest
	...
	/*-----------------------------------------------------------------
	 04. GET PAYMENT REQUESTS AND GET ERROR IF IS AVAILABLE
	-------------------------------------------------------------------*/
	paymentTransactions, err := service.EventHubPaymentService.GetOtherPaymentsByPagination(pagination, request.Query)
	/*---------------------------------------------------------
	 05. CHECK IF ERROR IS AVAILABLE AND RETURN ERROR RESPONSE
	----------------------------------------------------------*/
	if err != nil {
