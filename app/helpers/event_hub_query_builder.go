...
func (q eventHubQueryBuilder) QueryGetDashboardStatisticsForEventPlanner() string {
	return "SELECT " +
		...
		"(SELECT SUM(amount) FROM event_hub_payment_transactions LEFT JOIN event_hub_events WHERE event_hub_payment_transactions.payment_status='COMPLETED' AND event_hub_events.user_id = ?) AS total_amount"
}
